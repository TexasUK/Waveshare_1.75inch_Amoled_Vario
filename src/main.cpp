// main.cpp — Display + Audio (ES8311 + I2S) + Vario beeper + CST92xx touch
...
static inline bool valid_gpio(int pin) { 
  return pin >= 0 && pin <= 48; 
}
...
static bool touch_begin() {
  ...
  // Give display more time to initialize first
  delay(500);
  ...
  // Initialize I2C
  Serial.printf("[TOUCH] I2C: SDA=%d, SCL=%d @400kHz\n", I2C_SDA_PIN, I2C_SCL_PIN);
...
static void drawRoundedRect(TFT_eSprite& s, int x, int y, int w, int h, int r, uint16_t color) {
  // Draw rounded rectangle by drawing circles at corners and filling rectangles
...
static void drawSlider() {
  ...
  // Draw slider track background (white border)
  ...
  if (filled_width > 0) {
    // Draw filled portion (blue interior)
  ...
  // Draw slider handle (round button with white border and blue interior)
  spr.fillCircle(handle_x + 15, handle_y, 12, C_WHITE);
  spr.fillCircle(handle_x + 15, handle_y, 10, C_BLUE);
  
  // Value display
  char value_str[16];
  snprintf(value_str, sizeof(value_str), "%d", g_slider.value);
...
static void updateSliderFromTouch(int16_t x, int16_t y) {
  ...
  if (new_value != g_slider.value) {
    ...
    switch (g_settings_screen.selected_setting) {
      case 0: // Volume
        settingsManager.setVolume(g_slider.value);
        audioManager.setVolume(g_slider.value);
        break;
      case 1: // Brightness
        settingsManager.setBrightness(map(g_slider.value, 0, 10, 0, 255));
...
static void setupSlider() {
  g_slider.x_pos = 40;
  g_slider.y_pos = 280;
  g_slider.width = LCD_W - 80;
  g_slider.active = true;
  
  switch (g_settings_screen.selected_setting) {
    case 0: // Volume
      g_slider.min_value = 0;
      g_slider.max_value = 10;
      g_slider.value = settingsManager.settings.audio_volume;
      break;
    case 1: // Brightness
      g_slider.min_value = 0;
      g_slider.max_value = 10;
      g_slider.value = map(settingsManager.settings.display_brightness, 0, 255, 0, 10);
      break;
    case 2: // Units - Special case, we'll handle differently
...
public:
  ...
  void draw(TFT_eSprite& s) {
    ...
    const float f1k          = (hundredsVal >= 9.0f && tensValue >= 9.0f && unitsValue >= 9.0f)
                                ? (unitsValue - 9.0f) : 0.0f;
    ...
    s.setTextColor(C_LGREY, C_BLACK);
    s.setTextSize(2);
    s.setCursor(x2 + smallW + 10, ySmall + smallH - 20);
    s.print("ft");
  }
private:
  float alt_ = 0.f;
  ...
  static inline bool fullyInside(int topY, int textSize, int innerY, int innerH) {
    const int chH = 8 * textSize;
    return (topY >= innerY) && (topY + chH <= innerY + innerH);
  }
  
  static void drawColumn(TFT_eSprite& s, int x, int y, int w, int h, int textSize,
                         int currentDigit, float frac)
  {
    const int frameT = 4;
    const int ix = x + frameT, iy = y + frameT;
    const int iw = w - 2 * frameT, ih = h - 2 * frameT;

    s.fillRect(ix, iy, iw, ih, C_BLACK);

    int dCur  = (currentDigit % 10 + 10) % 10;
    int dPrev = (dCur + 9) % 10;
    int dNext = (dCur + 1) % 10;

    const int chW = 6 * textSize;
    const int chH = 8 * textSize;

    const int xText = ix + (iw - chW) / 2;
    const int yCenter = iy + (ih - chH) / 2;

    if (frac < 0) { frac = 0; }
    if (frac > 1) { frac = 1; }
    const int roll = (int)lroundf(frac * chH);

    const int yPrevTop = yCenter - chH - roll;
    const int yCurTop  = yCenter - roll;
    const int yNextTop = yCenter + chH - roll;

    s.setTextWrap(false);
    s.setTextColor(C_WHITE, C_BLACK);
...
public:
  ...
  void draw(TFT_eSprite& s) {
    ...
    s.setTextWrap(false);
    s.setTextColor(C_WHITE, C_BLACK);
    s.setTextSize(ASI_FONT);
    s.setCursor(ASI_NUM_X_, ASI_Y); s.print(fld);
    int ktsY = ASI_Y + (ASI_CHAR_H - ASI_KTS_CHAR_H) / 2;
    s.setTextSize(ASI_KTS_FONT);
    s.setCursor(ASI_KTS_X_, ktsY); s.print("kts");
  }
private:
...
static void drawMainScreen(float v, float alt_ft, float asi_kts_now) {
  ...
  gAlt.update(alt_ft); 
  gAlt.draw(spr);
  gASI.update(asi_kts_now); 
  gASI.draw(spr);
  
  // Draw touch status indicator
  ...
  spr.setTextSize(1);
  spr.setCursor(10, LCD_H - 20);
  spr.print("Swipe L->R for Settings");
...
static void drawSettingsScreen() {
  ...
  // Settings screen header - Centered
  spr.setTextColor(C_YELLOW, C_BLACK);
  spr.setTextSize(4);
  drawTextCentered(spr, "SETTINGS", LCD_W/2, 30, 4, C_YELLOW);
  
  // Highlight selected setting with larger fonts
  uint16_t bg_color = C_BLACK;
  uint16_t text_color = C_WHITE;
  
  // Audio Volume - Centered
  char volume_text[32];
  snprintf(volume_text, sizeof(volume_text), "Audio Volume: %d", settingsManager.settings.audio_volume);
  if (g_settings_screen.selected_setting == 0) {
    text_color = C_BLUE;
  } else {
    text_color = C_WHITE;
  }
  drawTextCentered(spr, volume_text, LCD_W/2, 90, 3, text_color);
  
  // Display Brightness - Centered
  char brightness_text[32];
  snprintf(brightness_text, sizeof(brightness_text), "Brightness: %d", map(settingsManager.settings.display_brightness, 0, 255, 0, 10));
  if (g_settings_screen.selected_setting == 1) {
    text_color = C_BLUE;
  } else {
    text_color = C_WHITE;
  }
  drawTextCentered(spr, brightness_text, LCD_W/2, 140, 3, text_color);
  
  // Units - Centered
  char units_text[32];
  snprintf(units_text, sizeof(units_text), "Units: %s", settingsManager.settings.use_metric ? "METRES" : "FEET");
  if (g_settings_screen.selected_setting == 2) {
    text_color = C_BLUE;
  } else {
    text_color = C_WHITE;
  }
  drawTextCentered(spr, units_text, LCD_W/2, 190, 3, text_color);
  
  // Calibration - Centered
  if (g_settings_screen.selected_setting == 3) {
    text_color = C_BLUE;
  } else {
    text_color = C_WHITE;
  }
  drawTextCentered(spr, "Calibration", LCD_W/2, 240, 3, text_color);
  
  // Draw slider if a numeric setting is selected
  if (g_slider.active && (g_settings_screen.selected_setting == 0 || g_settings_screen.selected_setting == 1)) {
    drawSlider();
  }
  
  // Back instruction - Centered
  spr.setTextColor(C_LGREY, C_BLACK);
  spr.setTextSize(2);
  drawTextCentered(spr, "Swipe R->L to go back", LCD_W/2, LCD_H - 30, 2, C_LGREY);
...
static void handleSettingsTouch(int16_t x, int16_t y) {
  // Check if slider is active and touch is on slider
  if (g_slider.active && y >= g_slider.y_pos - 10 && y <= g_slider.y_pos + g_slider.height + 10) {
    updateSliderFromTouch(x, y);
    return;
  }
  
  // Check which setting was tapped (using centered positions)
  if (y >= 75 && y <= 105) {
    g_settings_screen.selected_setting = 0; // Volume
    setupSlider();
  } else if (y >= 125 && y <= 155) {
    g_settings_screen.selected_setting = 1; // Brightness
    setupSlider();
  } else if (y >= 175 && y <= 205) {
    g_settings_screen.selected_setting = 2; // Units
    settingsManager.setUseMetric(!settingsManager.settings.use_metric);
    g_slider.active = false;
  } else if (y >= 225 && y <= 255) {
    g_settings_screen.selected_setting = 3; // Calibration
...
void setup() {
  ...
  CO5300_init();
  delay(500); // Give display more time to initialize
  lcd_setRotation(0);
  lcd_brightness(settingsManager.settings.display_brightness); // Use saved brightness
  ...
  spr.setTextSize(3);
  spr.setCursor(LCD_W/2 - 80, LCD_H/2 - 20);
  spr.print("BOOTING...");
  lcd_PushColors(COL_OFF, 0, LCD_W, LCD_H, (uint16_t*)spr.getPointer());
  
  delay(1000); // Show boot screen
  ...
  if (g_touch_initialized) {
    Serial.println("[TOUCH] ✓ Touch initialized");
  } else {
  ...
  if (!audioManager.begin(settingsManager.settings.audio_volume)) {
...
void loop() {
  ...
  // Demo signals
  float v = (5.0f * KTS_TO_MPS) * sinf(t * 0.6f);
  float alt_ft = 3962.0f + 7.0f * sinf(t * 0.12f) + 0.5f * sinf(t * 0.6f);
  ...
  // Use AudioManager for vario beeper
  ...
  if (g_touch_initialized) {
    ...
    if (pressed && !gSwipe.active) {
      ...
      if (g_in_settings) {
        if (g_settings_screen.calibrating) {
          // Handle calibration touch
          g_settings_screen.calibrating = false;
        } else {
      ...
      // Touch continuing - only update position for slider
      gSwipe.xLast = x;
      gSwipe.yLast = y;
      
    } else if (!pressed && gSwipe.active) {
      // Touch ended - evaluate horizontal swipe for navigation
      int dx = gSwipe.xLast - gSwipe.x0;
      int dy = gSwipe.yLast - gSwipe.y0;
      uint32_t dt = now - gSwipe.t0;
      ...
      if (isLeftToRight && !g_in_settings) {
        g_in_settings = true;
        g_slider.active = false; // Reset slider when entering settings
      } else if (isRightToLeft && g_in_settings) {
        g_in_settings = false;
        g_slider.active = false; // Reset slider when leaving settings
      }
    }
  }
  ...
  if (g_in_settings) {
    ...
    drawMainScreen(v, alt_ft, asi_val);
  }

  // Debug info
  if (millis() - last_debug > 5000) {
    last_debug = millis();
    Serial.printf("[STATUS] Uptime: %.1fs, Touch: %s, Screen: %s\n", 
                  t, 
